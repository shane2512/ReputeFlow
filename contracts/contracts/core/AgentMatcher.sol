...
contract AgentMatcher is AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        if (_pythContract == address(0) || _admin == address(0)) {
    ...
    /**
     * @notice Select random agent from pool using block-based randomness
    ...
    function selectRandomAgent(
    ...
    ) external nonReentrant whenNotPaused returns (address selectedAgent) {
        ...
        // Use block-based randomness (simplified for deployment)
        ...
        uint256 randomIndex = uint256(randomness) % pool.qualifiedAgents.length;
        ...
        selectionHistory[selectionId] = SelectionRecord({
            ...
            providerRandomness: randomness,
            timestamp: block.timestamp,
            isWeighted: false
        });
        
        emit AgentSelected(poolId, selectedAgent, 0, randomness);
