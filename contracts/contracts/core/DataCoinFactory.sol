...
contract DataCoinFactory is ERC721, AccessControl, ReentrancyGuard, Pausable {
    ...
    struct DataCoin {
        ...
        DataAccessControl accessRules;  // Access control rules
    ...
    function createDataCoin(
        ...
        DataAccessControl calldata accessRules
    ...
    function purchaseAccess(uint256 coinId) external payable nonReentrant whenNotPaused {
        ...
        if (coin.coinId == 0) revert("DataCoin not found");
        if (!coin.isActive) revert("DataCoin not active");
        
        DataAccessControl memory rules = coin.accessRules;
     ...
     * @param coinId DataCoin identifier
     * @param newRules New access rules
     */
    function updateAccessRules(
        uint256 coinId,
        DataAccessControl calldata newRules
    ) external whenNotPaused {
        DataCoin storage coin = dataCoins[coinId];
        if (coin.coinId == 0) revert InvalidCoinId();
